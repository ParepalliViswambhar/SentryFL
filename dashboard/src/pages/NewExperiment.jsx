...
const defaultConfig = { experiment: { name: '', seed: 42 }, data: { dataset: 'SMD', data_source: 'synthetic', window_size: 100, stride: 1 }, model: { backbone: 'bert-base-uncased', hidden_dim: 768, dropout: 0.1 }, training: { num_rounds: 100, local_epochs: 5, batch_size: 32, learning_rate: 0.001 }, federated: { num_clients: 10, clients_per_round: 5, partition_strategy: 'iid', aggregation: 'fedavg' }, privacy: { enabled: true, epsilon: 1, delta: 0.00001 }, parameter_efficiency: { adms_enabled: true, selection_ratio: 0.05 } };
...
const NewExperiment = () => {
  ...
  const submit = async (event) => { event.preventDefault(); setMessage(''); if (!config.experiment.name.trim()) { setMessage('Experiment name is required.'); return; } const result = await dispatch(createExperiment(toApiConfig(config))); if (createExperiment.fulfilled.match(result)) { const newId = result.payload.id ?? result.payload.experiment_id; if (newId) navigate(`/experiments/${newId}`); } };
  ...
  return (
    <Box>
      ...
      <Paper component="form" onSubmit={submit} sx={{ p: { xs: 2, md: 3 } }}><Stack spacing={3}>
        ...
        <Box><Typography variant="h6">Dataset and model</Typography><Divider sx={{ my: 1 }} /><Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}><TextField select label="Dataset" value={config.data.dataset} onChange={(e) => setConfig(update(config, 'data', 'dataset', e.target.value))} fullWidth><MenuItem value="SMD">SMD</MenuItem><MenuItem value="NSL-KDD">NSL-KDD</MenuItem></TextField><TextField select label="Data source" value={config.data.data_source} onChange={(e) => setConfig(update(config, 'data', 'data_source', e.target.value))} helperText={config.data.data_source === 'synthetic' ? 'Generated demo data — runs with zero setup' : 'Requires downloaded dataset files'} fullWidth><MenuItem value="synthetic">Demo (synthetic)</MenuItem><MenuItem value="real">Real dataset</MenuItem></TextField>{field('data', 'window_size', 'Window size')}{field('data', 'stride', 'Stride')}</Stack><Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} mt={2}><TextField label="Backbone" value={config.model.backbone} onChange={(e) => setConfig(update(config, 'model', 'backbone', e.target.value))} fullWidth />{field('model', 'hidden_dim', 'Hidden dimension')}{field('model', 'dropout', 'Dropout')}</Stack></Box>
